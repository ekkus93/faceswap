...
class FaceIt:
    ...
    def _extract_frames(self, person, video):
        ...
        print("###self._video_path(video):", self._video_path(video))
        fixed_video_name = self._fix_video_name(self._video_path(video))
        print("###fixed_video_path(video):", fixed_video_name)
        video_clip = VideoFileClip(fixed_video_name)
    ...
    def _symlink_faces_for_model(self, person, video):
        ...
        if not os.path.exists(self._model_person_data_path(person)):
            os.makedirs(self._model_person_data_path(person), exists_ok=True)
        for face_file in os.listdir(self._video_faces_path(video)):
            ...
            print("face_file_path:", face_file_path)
            #os.symlink(face_file_path, target_file)
            shutil.copyfile(face_file_path, target_file)
